...
@njit
def delayed_fractions(index, neutron_fission_reaction, data):
    offset = neutron_fission_reaction["delayed_fractions_offset"]
    return data[offset + index]


@njit
def delayed_fractions_all(neutron_fission_reaction, data):
...
@njit
def delayed_fractions_last(neutron_fission_reaction, data):
    start = neutron_fission_reaction["delayed_fractions_offset"]
    size = neutron_fission_reaction["delayed_fractions_length"]
    end = start + size
    return data[end - 1]
...
@njit
def delayed_fractions_chunk(start, length, neutron_fission_reaction, data):
    start += neutron_fission_reaction["delayed_fractions_offset"]
    end = start + length
    return data[start:end]
